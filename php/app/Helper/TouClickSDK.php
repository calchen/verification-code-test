...
class TouClickSDK {
    ...
    public function check($sid, $checkAddress, $token, $userName = '', $userId = 0) {
        if (empty ($checkAddress) || empty ($token) || empty($this->privateKey) || empty($this->publicKey)) {
            exit ('参数为空');
        }
        ...
        if ($check['code'] == 0) {
            ...
            $sign = $this->getSign($checkParam, $this->privateKey);
            if ($sign == $check['sign']) {
                $res['code'] = 12;
                $res['message'] = '验证成功';
    ...
    public function callback($sid, $checkAddress, $token, $isLoginSucc) {
        if (empty ($checkAddress) || empty ($sid) || empty ($token) || empty($this->privateKey) || empty($this->publicKey)) {
            exit ('参数为空');
        }
        if (!preg_match('/^[\w\-]/', $checkAddress)) {
            exit('参数格式不正确');
